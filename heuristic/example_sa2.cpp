...
random_device rd;
mt19937 eng(rd());
inline int generateRandom(int start, int end) {
 ...
 * Seed000 : xxxxxxxx
 */
void solve00(Score& score) {
  int simulate_cnt = 0;
  while(true) {
    double p = (double)(clock() - TIMER_START) / CLOCKS_PER_SEC / TIME_LIMIT;
    if(1.0 <= p) {
      cout << "Time Limit :" << simulate_cnt << " ";
      score.showBestScore();
      break;
    }
    ...
    vector<P> new_score;
    rep(i, cnt) {
      new_score.emplace_back(make_pair(v[i], generateRandom(100, 9999)));
    }
...
int main() {
  ...
  // ダミーで点数を入れる
  rep(i, N) {
    scoreDetail[i] = generateRandom(100, 9999);
  }
